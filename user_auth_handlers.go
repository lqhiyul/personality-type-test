...
func (a *App) handleAuthRegister(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := a.userStore.GetUserByUsername(ctx, username); err == nil {
		writeJSONError(w, http.StatusConflict, "username is already registered")
		return
	...
	if _, err := a.userStore.GetUserByEmail(ctx, email); err == nil {
		writeJSONError(w, http.StatusConflict, "email is already registered")
		return
	} else if !errors.Is(err, sql.ErrNoRows) {
...
func (a *App) handleAuthLogout(w http.ResponseWriter, r *http.Request) {
	...
	if cookie, err := r.Cookie(userSessionCookieName); err == nil && cookie.Value != "" {
		a.userSessions.delete(cookie.Value)
	}
...
func (a *App) setUserSession(w http.ResponseWriter, userID int64) {
	token, expiresAt := a.userSessions.create(userID)
...
func (a *App) currentUserID(r *http.Request) (int64, bool) {
	...
	return a.userSessions.userID(cookie.Value)
}
