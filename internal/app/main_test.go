...
func newTestApp(t *testing.T) *App {
	...
	staticFS := os.DirFS(filepath.Join("..", "..", "web", "static"))
...
func performJSON(app *App, method, target string, body any, cookies ...*http.Cookie) *httptest.ResponseRecorder {
	...
	if testUnsafeMethod(method) {
		addCSRFToRequest(app, req, cookies...)
	}
...
func addCSRFToRequest(app *App, req *http.Request, cookies ...*http.Cookie) {
	if existing := findCookie(cookies, httpmiddleware.CSRFCookieName); existing != nil && existing.Value != "" {
		req.Header.Set(httpmiddleware.CSRFHeaderName, existing.Value)
		return
	}

	rec := httptest.NewRecorder()
	app.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/healthz", nil))
	cookie := findCookie(rec.Result().Cookies(), httpmiddleware.CSRFCookieName)
	if cookie == nil || cookie.Value == "" {
		return
	}
	req.AddCookie(cookie)
	req.Header.Set(httpmiddleware.CSRFHeaderName, cookie.Value)
}

func testUnsafeMethod(method string) bool {
	switch method {
